...
import com.spotify.mobius2.MobiusLoop.Controller;
...
public interface Connectable<I, O> {
   ...
   * consumer.
   *
   * <p>Must return a new {@link com.spotify.mobius2.Connection} that accepts incoming values. After
   * {@link com.spotify.mobius2.Connection#dispose()} } is called on the returned {@link
   * com.spotify.mobius2.Connection}, the connection must be broken, and no more values may be sent
   ...
   * it should throw a {@link com.spotify.mobius2.ConnectionLimitExceededException} if someone tries
   * to connect a second time before disposing of the first connection.
   *
   * @param output the consumer that the new connection should use to emit values
   * @return a new connection
   * @throws com.spotify.mobius2.ConnectionLimitExceededException should be thrown if there are too
   *     many concurrent connections to this Connectable; this should be caused by incorrect usage
   *     of the Connectable, and is considered an irrecoverable error
   */
  @Nonnull
