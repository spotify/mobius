...
public class MobiusLoop<M, E, F> implements Loop<M, E, F> {
  ...
  @Nonnull private final com.spotify.mobius2.MessageDispatcher<E> eventDispatcher;
  @Nonnull private final com.spotify.mobius2.MessageDispatcher<F> effectDispatcher;

  @Nonnull private final EventProcessor<M, E, F> eventProcessor;
  @Nonnull private final com.spotify.mobius2.Connection<F> effectConsumer;
  @Nonnull private final QueuingConnection<M> eventSourceModelConsumer;

  @Nonnull
  private final List<com.spotify.mobius2.functions.Consumer<M>> modelObservers =
  ...
  static <M, E, F> MobiusLoop<M, E, F> create(
      com.spotify.mobius2.Update<M, E, F> update,
      M startModel,
      Iterable<F> startEffects,
      Connectable<F, E> effectHandler,
      Connectable<M, E> eventSource,
      com.spotify.mobius2.runners.WorkRunner eventRunner,
      com.spotify.mobius2.runners.WorkRunner effectRunner) {

    return new MobiusLoop<>(
        new EventProcessor.Factory<>(
            com.spotify.mobius2.MobiusStore.create(
                Preconditions.checkNotNull(update), Preconditions.checkNotNull(startModel))),
        Preconditions.checkNotNull(startModel),
        Preconditions.checkNotNull(startEffects),
        Preconditions.checkNotNull(effectHandler),
        Preconditions.checkNotNull(eventSource),
        Preconditions.checkNotNull(eventRunner),
        Preconditions.checkNotNull(effectRunner));
  }
  ...
  private MobiusLoop(
    ...
    onEventReceived =
        DiscardAfterDisposeWrapper.wrapConsumer(
            new com.spotify.mobius2.functions.Consumer<E>() {
              @Override
    ...
    onEffectReceived =
        DiscardAfterDisposeWrapper.wrapConsumer(
            new com.spotify.mobius2.functions.Consumer<F>() {
              @Override
    ...
    com.spotify.mobius2.functions.Consumer<M> onModelChanged =
        new com.spotify.mobius2.functions.Consumer<M>() {
          @Override
          public void accept(M model) {
            mostRecentModel = model;
            eventSourceModelConsumer.accept(model);
            for (com.spotify.mobius2.functions.Consumer<M> observer : modelObservers) {
    ...
    this.eventDispatcher =
        new com.spotify.mobius2.MessageDispatcher<>(eventRunner, onEventReceived);
    this.effectDispatcher =
        new com.spotify.mobius2.MessageDispatcher<>(effectRunner, onEffectReceived);

    this.eventProcessor = eventProcessorFactory.create(effectDispatcher, onModelChanged);

    com.spotify.mobius2.functions.Consumer<E> eventConsumer =
        new com.spotify.mobius2.functions.Consumer<E>() {
          @Override
          public void accept(E event) {
            dispatchEvent(event);
  ...
  public void dispatchEvent(E event) {
    ...
    try {
      eventDispatcher.accept(Preconditions.checkNotNull(event));
  ...
  @Override
  public com.spotify.mobius2.disposables.Disposable observe(final Consumer<M> observer) {
    if (runState == RunState.DISPOSED) {
      throw new IllegalStateException(
    ...
    modelObservers.add(Preconditions.checkNotNull(observer));
  ...
  public interface Builder<M, E, F> extends Factory<M, E, F> {
     ...
     * @return a new {@link Builder} with the supplied {@link com.spotify.mobius2.Init}, and the
     *     same values as the current one for the other fields.
     */
    @Nonnull
    @Deprecated
    Builder<M, E, F> init(com.spotify.mobius2.Init<M, F> init);

    /**
     * @return a new {@link Builder} with the supplied {@link com.spotify.mobius2.EventSource}, and
     *     the same values as the current one for the other fields. NOTE: Invoking this method will
     *     replace the current {@link com.spotify.mobius2.EventSource} with the supplied one. If you
     *     want to pass multiple event sources, please use {@link
     *     #eventSources(com.spotify.mobius2.EventSource, com.spotify.mobius2.EventSource[])}.
     */
    @Nonnull
    Builder<M, E, F> eventSource(com.spotify.mobius2.EventSource<E> eventSource);

    /**
     * @return a new {@link Builder} with an {@link com.spotify.mobius2.EventSource} that merges the
     *     supplied event sources, and the same values as the current one for the other fields.
     */
    @Nonnull
    Builder<M, E, F> eventSources(
     ...
     *     its event source, it will connect to it and will invoke the {@link
     *     com.spotify.mobius2.Connection <M>} accept method every time the model changes. This
     *     allows us to conditionally subscribe to different sources based on the current state. If
     *     you provide a regular {@link EventSource <E>}, it will be wrapped in a {@link
     *     Connectable} and that implementation will subscribe to that event source only once when
  ...
  public interface Factory<M, E, F> {
     ...
     * @return the started {@link MobiusLoop}
     * @throws IllegalStateException if the loop has been configured with an {@link
     *     com.spotify.mobius2.Init}, since that would conflict with the initial effects passed in.
     */
  ...
  public interface Logger<M, E, F> {
    /**
     * Called right before the {@link com.spotify.mobius2.Init#init(Object)} function is called.
     *
    ...
    /**
     * Called right after the {@link com.spotify.mobius2.Init#init(Object)} function is called.
     *
     * <p>This method mustn't block, as it'll hinder the loop from running. It will be called on the
     * same thread as the init function.
     *
     * @param model the model that was passed to init
     * @param result the {@link com.spotify.mobius2.First} that init returned
     */
    ...
    /**
     * Called right before the {@link com.spotify.mobius2.Update#update(Object, Object)} function is
     * called.
     *
     * <p>This method mustn't block, as it'll hinder the loop from running. It will be called on the
    ...
    /**
     * Called right after the {@link com.spotify.mobius2.Update#update(Object, Object)} function is
     * called.
     *
     * <p>This method mustn't block, as it'll hinder the loop from running. It will be called on the
     * same thread as the update function.
