...
public class MobiusLoopTest {
  ...
  @Test
  public void eventSourceMayEmitEventsBeforeReturningFromDispose() throws Exception {
    // Event sources should be allowed to emit events before they return
    // from their dispose() method.

    final MobiusLoop.Builder<String, TestEvent, TestEffect> builder =
        Mobius.loop(update, effectHandler)
            .eventSource(new EmitDuringDisposeEventSource(new TestEvent("foo")));

    builder.startFrom("foo").dispose();
  }

  @Test
  public void effectHandlerMayEmitEventsBeforeReturningFromDispose() throws Exception {
    // Effect handlers should be allowed to emit events before they return
    // from their dispose() method.

    final MobiusLoop.Builder<String, TestEvent, TestEffect> builder =
        Mobius.loop(update, new EmitDuringDisposeEffectHandler());

    builder.startFrom("foo").dispose();
  }

  @Test
  public void updateMayEmitModelsBeforeReturningFromDispose() throws Exception {
    // Update should be allowed to emit model changes during dispose without anything
    // bad happening.
  ...
  @Test
  public void updateMayEmitEffectsBeforeReturningFromDispose() throws Exception {
    // Update should be allowed to emit effects during dispose without anything
    // bad happening.
